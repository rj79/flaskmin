...
based on the code created by singingwolfboy available here:
...
class FlaskClient(BaseFlaskClient):
    def __init__(self, *args, **kwargs):
        ...
        self.logout_url = kwargs.get('logout_url', '/logout')
        self.login_user_field = kwargs.get('login_user_field', 'username')
    ...
    def csrf_token(self):
        ...
        with flask.current_app.test_request_context(
                self.login_url, environ_overrides=environ_overrides,
            ):
    ...
    def login(self, user_id, password):
        return self.post(self.login_url, data={
            self.login_user_field: user_id,
            self.login_password_field: password,
            'csrf_token': self.csrf_token},
            follow_redirects=True)

    def logout(self):
        return self.get(self.logout_url, follow_redirects=True)
